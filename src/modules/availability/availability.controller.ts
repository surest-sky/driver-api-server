...
export class AvailabilityController {
  ...
  async listCoach(@Req() req: any, @Param("id") id: string) {
    ...
    const sameSchool =
      requester.schoolId != null && requester.schoolId === coach.schoolId;
  ...
  @Get("student/:id")
  async listStudent(@Req() req: any, @Param("id") id: string) {
    ...
    const sameSchool =
      requester.schoolId != null && requester.schoolId === student.schoolId;

    if (requester.role === UserRole.student) {
      if (requester.id !== studentId) {
        throw new ForbiddenException("无权查看该学员的不可用时间");
      }
    } else if (requester.role === UserRole.coach) {
      const isManager = Boolean((requester as any).isManager);
      if (!sameSchool && !isManager) {
        throw new ForbiddenException("无权查看该学员的不可用时间");
      }
    }

    return this.svc.listForUser(studentId);
  }
